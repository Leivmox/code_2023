...
int searchSequentialList(struct SequentialList *list, int target)
{
    for (int i = 0; i < list->size; i++)
    {
        ...
        {
            return i + 1; // 返回元素的位置（加1以匹配人类习惯的位置，从1开始）
        }
    }
    return -1; // 元素未找到，返回-1表示未找到
}
...
// 合并两个有序顺序表
void mergeSequentialLists(struct SequentialList *list1,
                          struct SequentialList *list2, struct SequentialList *result)
{
    int i = 0, j = 0;
